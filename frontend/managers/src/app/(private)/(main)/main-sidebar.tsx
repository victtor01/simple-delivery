...
const MainSidebar = () => {
  ...
  return (
    <section className="w-full max-w-[15rem] h-full border-r bg-white flex">
      <div className="flex flex-col w-full">
        <section className="flex-1 flex flex-col">
          <header
            className="text-gray-700 font-semibold text-lg
            px-3 mt-2 capitalize"
          >
            ...
            {Object.entries(links).map(([name, { link, icon: Icon }]) => {
              ...
              return (
                <Link
                  ...
                  text-md text-gray-500 rounded-md transition-all ${styleSelected}`}
                >
                  <Icon />
                  {name}
                </Link>
        ...
        <footer
          className="p-2 border-t bg-gray-50 flex 
